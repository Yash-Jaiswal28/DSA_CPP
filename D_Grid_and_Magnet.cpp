...
void dfs(vector<vector<char>>& v,vector<vector<ll>>& vis,ll i,ll j,ll n,ll m,set<pair<ll,ll>>& st){
    //if(vis[i][j]==2)return ;
    vis[i][j]=1;
    for(ll k=0;k<4;k++){
        ll x=i+dx[k];
        ...
        if(x>=0 && x<n && y>=0 && y<m){
            if(vis[x][y]==0){
                st.insert({x,y});
                dfs(v,vis,x,y,n,m,st);
            }else if(vis[x][y]==2){
                st.insert({x,y});
           // dfs(v,vis,i,j,n,m,st);
            }
        }
    }
}
      ...
      for(ll i=0;i<n;i++){
        for(ll j=0;j<m;j++){
            if(vis[i][j]==0){
                ...
                st.insert({i,j});
                dfs(v,vis,i,j,n,m,st);
                // for(auto it:st)cout<<it.ff<<" -> "<<it.ss<<"   ";
                // cout<<endl;
