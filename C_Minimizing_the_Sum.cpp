...
ll fun(vector<ll>& v,vector<vector<ll>>& dp,ll i,ll k,ll n){
    if(i>=n)return 0;
    if(dp[i][k]!=-1)return dp[i][k];
    
    ll not_take=fun(v,dp,i+1,k,n);
    ll take=0,sum=v[i],mn=v[i];
    for(ll j=i+1;j<min(n,i+1+k);j++){
        mn=min(v[j],mn);
        sum+=v[j];
        ll temp=sum-mn*(j-i+1)+fun(v,dp,j+1,k-(j-i),n);
        take=max(take,temp);
    }
