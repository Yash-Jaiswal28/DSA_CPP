...
int main() {
  ...
  cin>>t;
    for(ll Z=0;Z<t;Z++)
    {
      ...
      ll gap1=(x1-x2);
      ll gap2=(y1-y2);
      //cout<<gap1<<" "<<gap2<<" "<<w-y1<<" "<<y1-1<<" "<<w-y2<<" "<<y2-1<<endl;
      if(gap1<0){
            if(abs(gap1)&1){
              if(abs(gap2)>(abs(gap1)+1)/2){
                cout<<"Draw"<<Endl;
              }else{
                if(abs(gap2)<=1)cout<<"Alice"<<Endl;
                else{
                  if(y1<y2){
                    if(w-y1<=(abs(gap1)+1)/2)cout<<"Alice"<<endl;
                    else cout<<"Draw"<<Endl;
                  }else{
                    if(y1-1<=(abs(gap1)+1)/2)cout<<"Alice"<<endl;
                    else cout<<"Draw"<<Endl;
                  }
                }
              }
            }else{
              if(abs(gap2)>(abs(gap1))/2){
                cout<<"Draw"<<Endl;
              }else{
                if(abs(gap2)==0)cout<<"Bob"<<Endl;
                else{
                  if(y1<y2){
                    if(y2-1<=(abs(gap1))/2)cout<<"Bob"<<endl;
                    else cout<<"Draw"<<Endl;
                  }else{
                    if(w-y2<=(abs(gap1))/2)cout<<"Bob"<<endl;
                    else cout<<"Draw"<<Endl;
                  }
                }
              }
