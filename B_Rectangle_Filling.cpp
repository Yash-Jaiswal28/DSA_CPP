...
int main() {
  ...
  cin>>t;
    for(ll Z=0;Z<t;Z++)
    {
      ...
      if(n==1){
        if(v[0][m-1]==v[0][0])yes;
        else no;
      }else if(m==1){
        if(v[0][0]==v[n-1][0])yes;
        else no;
      }else
      if(s2.find(v[0][0])!=s2.end() && s3.find(v[0][0])!=s3.end())yes;
      else if(s4.find(v[0][m-1])!=s4.end() && s3.find(v[0][m-1])!=s3.end())yes;
      else if(s4.find(v[n-1][m-1])!=s4.end() && s1.find(v[n-1][m-1])!=s1.end())yes;
      else if(s2.find(v[n-1][0])!=s2.end() && s1.find(v[n-1][0])!=s1.end())yes;
      else no;
    }
  }
/*
