...
void solve(){
    ...
    for(auto it:mp){
        ll sum1=it.ff;
        vector<pair<ll,ll>> pairs1=it.ss;
        ll ck=x-sum1;
        if(mp.count(ck)){
            vector<pair<ll,ll>> pairs2=mp[ck];
            for(auto it1:pairs1){
                for(auto it2: pairs2){
                    if(it1.ff!=it2.ff && it1.ff!=it2.ss && it1.ss!=it2.ff && it1.ss !=it2.ss){
                        cout<<it1.ff<<" "<<it1.ss<<" "<<it2.ff<<" "<<it2.ss;
                        return;
                    }
                }
            }
