...
ll dfs(ll root, pair<ll, ll> adj[], string &s, ll n, vector<ll> &memo) {
    if (root == 0) return 0;
    if (memo[root] != INT_MAX) return memo[root];

    ll ans = n + 10;
    if (adj[root].ff) ans = min(ans, dfs(adj[root].ff, adj, s, n, memo) + (s[root - 1] != 'L'));
    if (adj[root].ss) ans = min(ans, dfs(adj[root].ss, adj, s, n, memo) + (s[root - 1] != 'R'));

    if(ans==n+10)ans=0;
    memo[root] = min(ans,memo[root]);
    return ans;
}
  ...
  cin>>t;
    for(ll Z=0;Z<t;Z++)
    {
      ll n;
        ...
        vector<ll> memo(n + 1, INT_MAX);
        cout << dfs(1, adj, s, n, memo) << Endl;
    }
  }
/*
Hey, you look strong..!!
