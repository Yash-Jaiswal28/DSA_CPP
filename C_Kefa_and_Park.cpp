...
ll ans=0;
ll fun(vector<ll> adj[],vector<ll>& vis,vector<ll>& v,vector<ll>& dp,ll node,ll n,ll m,ll cr){
    vis[node]=1;
    if(cr>m)return 0;
    for(auto it:adj[node]){
        if(vis[it]==0){
            vis[it]=1;
            if(v[it]==0)dp[it]=0;
            else dp[it]+=dp[node]+1;
            ll e =fun(adj,vis,v,dp,it,n,m,dp[it]);
            if(e && adj[it].size()==1){
                //cout<<node<<" "<<it<<" "<<dp[it]<<endl;
            ans+=e;
            }
        }
    }
...
//     for(ll Z=0;Z<t;Z++)
    {
      ...
      vector<ll> dp(n);
      dp[0]=v[0];
      fun(adj,vis,v,dp,0,n,m,dp[0]);
       //display(dp);
    //   for(auto it:adj)display(it);
      cout<<ans<<endl;
    }
  }
/*
