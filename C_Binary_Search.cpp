...
ll fact(ll n){if(n<2)return 1LL;else{ll a=1;for(ll i=2;i<=n;i++){a=((a%mod)*(i%mod))%mod;}return a;}}
...
ll nPr(ll n,ll r){if(r>n)return 0;ll d=fact(n);ll c=fact(n-r);return (d/c);}
...
void solve(){
    ...
    ll total_jyada=n-num;
    ll total_kam=num-1;
    ll rest=n-jyada-kam-1;
   // cout<<total_jyada<<" "<<jyada<<endl<<total_kam<<" "<<kam<<endl<<rest<<endl;
   ll ans=1;
   for(ll i=1;i<=kam;i++){
    ans=(ans*total_kam)%mod;
    total_kam--;
   }
    for(ll i=1;i<=jyada;i++){
        ans=(ans*total_jyada)%mod;
        total_jyada--;
    }
    for(ll i=1;i<=rest;i++){
        ans=(ans*i)%mod;
