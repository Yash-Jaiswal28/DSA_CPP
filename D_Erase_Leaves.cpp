...
ll fun(vector<ll> adj[], vector<ll> &vis, vector<ll> &ord, ll &a, ll node)
{
  vis[node] = 1;
...
int main()
{
  ...
  //     for(ll Z=0;Z<t;Z++)
  {
    ...
    {
      ll e = ord[it] - 1;
      hat.pb(fun(adj, vis, ord, e, it));
    }
    sort(hat.begin(), hat.end());
    for (ll i = 0; i < hat.size() - 1; i++)
      d += hat[i];
    cout << a + d;
  }
}
