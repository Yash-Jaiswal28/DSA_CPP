...
map<ll,ll> mp;
ll fun(vector<ll>& v,vector<ll>& dp,ll ind,ll n){
    if(ind>=n)return 0;
    if(dp[ind]!=-1)return dp[ind];
    ll ans=fun(v,dp,ind+1,n);
    if(ind<n-1){
        ans=max(ans,v[ind]*mp[v[ind]]+fun(v,dp,ind+1+(v[ind]+1==v[ind+1]),n));
    }else ans=max(ans,mp[v[ind]]*v[ind]+fun(v,dp,ind+1,n));
    return dp[ind]=ans;
}
void solve(){
    ll n;
